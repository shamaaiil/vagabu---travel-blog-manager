...
const PendingOrders = () => {
    ...
    if (ordersData) {
      ...
      if (Array.isArray(ordersData.data)) {
        const pendingOrders = ordersData.data.filter((order) => {
          return order.status && order.status.toLowerCase() === "pending"; // Filter orders where status is "pending"
        });
        ...
        const formattedOrders = pendingOrders.map((order) => {
          ...
          return {
            ...
            user_id: order?.customer?.full_name,
  ...
  return (
    <div className="mt-10 mb-8 flex bg-[#F2F7FB] flex-col gap-12 px-8">
      <DynamicTable

...
const OrderDisputes = () => {
  ...
  const { data: supportTicketsResponse, isLoading: ticketsLoading } =
    useGetQuery({
      path: "/support-tickets?source_type='disputes'",
      tag: ["SupportTickets"],
    });
  ...
  const handleSubmitUpdate = async (e) => {
    ...
    try {
      ...
      await updateStatus({
        path: `admin/support-tickets/${selectedTicketUuid}`,
        body: {
          assigned_to: assignee,
          status,
          priority,
    ...
    <>
      <div className="flex flex-col md:flex-row p-2 md:p-4 min-h-screen">
          ...
          <div className="space-y-3">
            {ticketsLoading ? (
              ...
              filteredTickets.map((ticket) => (
                ...
                >
                  <div className="text-xs w-6 md:w-8 h-6 md:h-8 flex-shrink-0 bg-gray-200 rounded-md text-center flex items-center justify-center text-gray-600 mr-2">
                    {ticket.id}
                  </div>
        ...
        <div
          ...
          } md:block md:w-2/3 bg-white rounded-xl shadow-sm flex flex-col h-[500px] md:h-auto relative`}
        >
          {selectedTicketUuid ? (
            <>
              {/* Chat Header */}
